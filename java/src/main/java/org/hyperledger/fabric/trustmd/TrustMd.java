...
import org.hyperledger.fabric.trustmd.data.TrustAsset;
...
public class TrustMd extends ChaincodeBase {
    ...
    public Response init(ChaincodeStub stub) {
        try {
            _logger.info("Init java dcTrustMdContract");
            ...
            final TrustAsset trustAsset = buildTrustAsset(args);
    ...
    private TrustAsset buildTrustAsset(final List<String> args) {

        final String clusterId = args.get(0);
        final String mecHostId = args.get(1);
        final String evaluatedNodeId = args.get(2);
        final String trustDecision =  args.get(3);
        final String degreeOfBelief = args.get(4);

        _logger.info("Creating trust asset");
        // Initialize the chaincode
        final TrustAsset asset = new TrustAsset(clusterId,
                evaluatedNodeId, trustDecision,
                degreeOfBelief, mecHostId);

        _logger.info(String.format("clusterId %s; evaluatedNodeId %s, trustDecision %s, degreeOfBelief %s, mecHostId %s",
                clusterId, evaluatedNodeId, trustDecision, degreeOfBelief, mecHostId)
        );
    ...
    private Response updateTrustDecisionAndValue(final ChaincodeStub stub, final List<String> args) {
        ...
        try {
            final TrustAsset newTrust = buildTrustAsset(args);
